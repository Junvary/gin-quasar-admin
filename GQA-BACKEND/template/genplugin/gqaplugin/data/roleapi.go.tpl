...
func (s *sysRoleApi) LoadData() error {
	return gqaGlobal.GqaDb.Transaction(func(tx *gorm.DB) error {
		...
		if count != 0 {
			...
			gqaGlobal.GqaSLogger.Warn("[GQA-plugins] --> sys_role_api 表中{{.PluginCode}}插件数据已存在，跳过初始化数据！", "has_count", count)
			return nil
		}
		...
		gqaGlobal.GqaSLogger.Info("[GQA-plugins] --> {{.PluginCode}}插件初始数据进入 sys_role_api 表成功！")
		return nil
	})
}
