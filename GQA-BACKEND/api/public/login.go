...
func (a *ApiLogin) Login(c *gin.Context) {
	...
	if global.Store.Verify(l.CaptchaId, l.Captcha, true) {
		...
		if err, user := servicePublic.ServiceLogin.Login(u); err != nil {
			global.GqaSLogger.Error(l.Username + utils.GqaI18n(c, "LoginFailed"))
			model.ResponseErrorMessage(utils.GqaI18n(c, "LoginFailed"), c)
		...
		model.ResponseErrorMessage(utils.GqaI18n(c, "CaptchaError"), c)
...
func (a *ApiLogin) createToken(user model.SysUser, c *gin.Context) {
	...
	if ss == "" {
		global.GqaSLogger.Error(utils.GqaI18n(c, "JwtConfigError"))
		model.ResponseErrorMessage(utils.GqaI18n(c, "JwtConfigError"), c)
		return
	}
