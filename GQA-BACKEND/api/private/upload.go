...
func (a *ApiUpload) UploadAvatar(c *gin.Context) {
	...
	if err != nil {
		global.GqaSLogger.Error(utils.GqaI18n(c, "ParseFileFailed"), "err", err)
		model.ResponseErrorMessage(utils.GqaI18n(c, "ParseFileFailed")+err.Error(), c)
		return
	}
	err, avatarUrl := servicePrivate.ServiceUpload.UploadAvatar(username, avatar, avatarHeader)
	if err != nil {
		global.GqaSLogger.Error(utils.GqaI18n(c, "UploadFailed"), "err", err)
		model.ResponseErrorMessage(utils.GqaI18n(c, "UploadFailed")+err.Error(), c)
	} else {
